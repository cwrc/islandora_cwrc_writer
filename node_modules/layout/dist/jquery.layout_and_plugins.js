...
// DO NOT CHANGE the zIndex values below unless you clearly understand their relationships
        ...
        , panes: {// default options for 'all panes' - will be overridden by 'per-pane settings'
            applyDemoStyles: false  // NOTE: renamed from applyDefaultStyles for clarity
                    ...
                    //Add responsiveness
            , responsive: false //is the pane responsive
            , responsiveWhen: "md" //when the pane should be responsive
            , responsiveSizes: {
                xl: 1140,
                lg: 992,
                md: 768,
            ...
            , onresize_end: function () {}  // CALLBACK when pane ENDS being Resized ***FOR ANY REASON***
...
//, hasParentLayout = false - exists ONLY inside Instance so can be set externally
        ...
        , _fixIframe = function (pane) {
            if (browser.mozilla)
                return; // skip FireFox - it auto-refreshes iframes onShow
            var $P = $Ps[pane];
        ...
        , isInitialized = function () {
            if (state.initialized || state.creatingLayout)
                ...
                return _initLayoutElements(); // try to init panes NOW
        }
        ...
        , _initLayoutElements = function (retry) {
            ...
            if (o.scrollToBookmarkOnLoad) {
                var l = self.location;
                if (l.hash)
                    l.replace(l.hash); // scrollTo Bookmark
            }
        ...
        , setInstanceKey = function (child, parentPaneState) {
            ...
            var $c = child.container
                    ...
                    ;
            if (!key)
                key = (sm && sm.cookie ? sm.cookie.name : '') || o.name; // look for a name/key
            if (!key)
                key = "layout" + (++parentPaneState.childIdx); // if no name/key found, generate one
            else
                key = key.replace(/[^\w-]/gi, '_').replace(/_{2,}/g, '_');  // ensure is valid as a hash key
            o.instanceKey = key;
        ...
        , refreshChildren = function (pane, newChild) {
            ...
            if (newChild) {
                ...
                if (!pC)
                    pC = children[pane] = {}; // create an empty children hash
        ...
        , addPane = function (pane, force) {
            ...
            if (dir != "horz")
                sizeMidPanes(pane, true); // true = skipCallback
        ...
        , setAsClosed = function (pane) {
            if (!$Rs[pane])
                return; // handles not initialized yet!
            var
                    $P = $Ps[pane]
                    , $R = $Rs[pane]
                    , $T = $Ts[pane]
        ...
        , sizeMidPanes = function (panes, skipCallback, force) {
            ...
            $.each(panes, function (i, pane) {
                ...
                if (pane === "center") {
                    ...
                    if (!hasRoom)
                        s.noVerticalRoom = true; // makePaneFit() logic
                }
                ...
                if (hasRoom) {
                    ...
                    if (s.isVisible) {
                        ...
                        if (state.initialized)
                            sizeContent(pane); // also resize the contents, if exists
                    }
        ...
        , resizeAll = function (evt_or_refresh) {
            ...
            $.each(["south", "north", "east", "west"], function (i, pane) {
                ...
                {
                    //if(s.size >= o.responsiveSizes.lg)
                    if (windowWidth >= o.responsiveSizes.lg)
                        if (o.responsiveWhen === 'lg' || o.responsiveWhen === 'md' || o.responsiveWhen === 'sm' || o.responsiveWhen === 'xs')
                        {
                    ...
                    //if(s.size >= o.responsiveSizes.md)
                    if (windowWidth >= o.responsiveSizes.md && windowWidth < o.responsiveSizes.lg)
                        if (o.responsiveWhen === 'md' || o.responsiveWhen === 'sm' || o.responsiveWhen === 'xs')
                        {
                            paneResponsive = false;
                        } else
                        {
                    ...
                    if (windowWidth >= o.responsiveSizes.sm && windowWidth < o.responsiveSizes.md)
                        //if(s.size >= o.responsiveSizes.sm)
                        if (o.responsiveWhen === 'sm' || o.responsiveWhen === 'xs')
                        {
                            paneResponsive = false;
                        } else
                        {
                    ...
                    //if(s.size >= o.responsiveSizes.xs)
                    if (windowWidth >= o.responsiveSizes.xs && windowWidth < o.responsiveSizes.sm)
                        if (o.responsiveWhen === 'xs')
                        {
                            paneResponsive = false;
                        } else
                        {
        ...
        , sizeContent = function (evt_or_panes, remeasure) {
            ...
            $.each(panes, function (idx, pane) {
                ...
                if (!$C.length) {
                    ...
                    if (!$C)
                        return;   // no replacement element found - pointer have been removed
                }
...
// Set stateManagement as a layout-option, NOT a pane-option
    ...
    $.layout.buttons = {
        ...
        , _load: function (inst) {
            ...
            $.extend(inst, {
                bindButton: function (selector, action, pane) {
                    return $.layout.buttons.on(inst, selector, action, pane);
                }
                // DEPRECATED METHODS...
                ...
                , addCloseBtn: function (selector, pane) {
